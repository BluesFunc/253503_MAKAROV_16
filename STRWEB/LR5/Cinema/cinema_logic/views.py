...
def user_orders(request):
    tickets = Ticket.objects.filter(order__customer__user=request.user).select_related('show','show__movie','order')
    orders = tickets.filter(order__is_paid=True)
    cart_items = tickets.filter(order__is_paid=False)
    shows = {}
    items = {}
    for order in orders:
        show = order.show
    ...
    return render(request, 'user_orders.html', {'shows': shows, 'items': items})
...
def show_order_tickets(request, show_id):
    show = get_object_or_404(Show, id=show_id)
    tickets = Ticket.objects.filter(show=show, order__customer__user=request.user, order__is_paid=True)
